...
# 1. Update services_layout.html for ultra-compact above-the-fold layout
...
<style>
    ...
    /* Compact Vertical Node Diagram Styling (Fits in 320px height) */
    .compact-v-diagram {
        padding: 20px 22px;
        border-radius: 16px;
        background: rgba(5, 10, 22, 0.92);
        border: 1px solid rgba(0, 210, 255, 0.2);
        box-shadow: 0 10px 30px rgba(0,0,0,0.5), inset 0 0 20px rgba(0, 210, 255, 0.03);
        display: flex;
...
<section class="reveal" style="padding: 110px 0 30px 0; background: radial-gradient(circle at top center, rgba(0, 210, 255, 0.06) 0%, transparent 70%); text-align: center; border-bottom: 1px solid rgba(255,255,255,0.05);">
    <div class="container">
        ...
        <p class="subtext mx-auto" style="font-size: 14px; max-width: 580px; margin-bottom: 0;">
            Select a service category below to inspect specs, real system architecture designs, and transparent pricing.
        </p>
...
def generate_service_page(service):
    ...
    # Compact Vertical Real System Nodes HTML
    cv_nodes_html = ""
    for idx, (v_title, v_desc, v_icon) in enumerate(service["vertical_nodes"]):
        cv_nodes_html += f'''
        <div class="cv-node">
            <div class="cv-node-icon">{v_icon}</div>
            <div>
                <div style="font-size: 13px; font-weight: 700; color: #fff;">{v_title}</div>
                <div style="font-size: 11px; color: var(--c-text-secondary);">{v_desc}</div>
            </div>
        </div>'''
        if idx < len(service["vertical_nodes"]) - 1:
            cv_nodes_html += '\n        <div class="cv-connector"></div>'
...
<div style="min-height: 100vh;">
    ...
    <!-- 1. ULTRA-COMPACT ABOVE-THE-FOLD SERVICE OVERVIEW SPLIT LAYOUT -->
    <section class="reveal" style="padding: 40px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
        <div class="container">
            <div style="display: grid; grid-template-columns: 1.15fr 0.85fr; gap: 40px; align-items: center;">
                ...
                <!-- LEFT SIDE (HEADING + CONCISE ABOUT + PILLARS) -->
                <div>
                    <div style="margin-bottom: 16px;">
                        <span class="overline highlight" style="font-size: 11px;">Service Architecture</span>
                        <div style="display: flex; align-items: center; gap: 12px; margin-top: 8px;">
                            <span style="font-size: 38px;">{service["icon"]}</span>
                            <h2 class="text-gradient" style="font-size: clamp(28px, 3.5vw, 38px); font-weight: 800; margin: 0; line-height: 1.15;">
                                {service["title"]}
                            </h2>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        <span style="padding: 5px 12px; border-radius: 20px; background: rgba(0, 210, 255, 0.08); border: 1px solid rgba(0, 210, 255, 0.2); color: #00d2ff; font-size: 11px; font-weight: 600;">⚡ Async Microservices</span>
                        <span style="padding: 5px 12px; border-radius: 20px; background: rgba(138, 35, 137, 0.08); border: 1px solid rgba(138, 35, 137, 0.2); color: #c07bc5; font-size: 11px; font-weight: 600;">🛡️ Private Data Security</span>
                        <span style="padding: 5px 12px; border-radius: 20px; background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.1); color: #fff; font-size: 11px; font-weight: 600;">🧠 Contextual Memory</span>
                    </div>
                </div>

                <!-- RIGHT SIDE: COMPACT REAL SYSTEM DESIGN VERTICAL DIAGRAM -->
                <div class="compact-v-diagram">
                    <div style="font-family: var(--font-display); font-size: 10px; font-weight: 700; color: var(--c-accent-cyan); text-transform: uppercase; letter-spacing: 1.5px; text-align: center;">
                        REAL SYSTEM ARCHITECTURE FLOW
                    </div>
                    {cv_nodes_html}
                </div>
...
# 5 Detailed Services Data with REAL SYSTEM ARCHITECTURE VERTICAL NODES
services_data = [
    {
        ...
        "vertical_nodes": [
            ("Client Prompt & Webhook Ingress", "REST payload / Natural language input", "👤"),
            ("LangGraph State Agent Planner", "Multi-agent DAG task decomposition", "🧠"),
            ("Pinecone Vector & Redis Memory", "Long-term state & context store", "🗄️"),
            ("FastAPI Function & Tool Execution", "Dispatches CRM API & DB calls", "⚡")
        ],
        "backend_steps": [
...
<span style="color:#00d2ff">[EXEC]</span> Action Executed in 142ms. Output written to Postgres DB.''',
    ...
    },
    {
        ...
        "vertical_nodes": [
            ("Unstructured Silo Ingestion", "PDFs, Confluence pages, & SQL data chunking", "📄"),
            ("text-embedding-3 Hybrid Index", "BM25 keyword + 3072-dim dense embeddings", "📉"),
            ("Pinecone / Qdrant Vector Store", "Sub-10ms HNSW cosine similarity index", "🗄️"),
            ("Grounded Llama 3 / GPT-4 LLM", "Synthesizes answer backed by citations", "💬")
        ],
        "backend_steps": [
...
<span style="color:#27c93f">[STREAM]</span> Streaming Grounded Answer to Client (Zero Hallucination)''',
    ...
    },
    {
        ...
        "vertical_nodes": [
            ("Streaming Telemetry Ingress", "IoT, transactions, & 60 FPS video streams", "📦"),
            ("Automated Feature Store", "Data normalization & tensor batching", "🧹"),
            ("NVIDIA TensorRT GPU Engine", "Sub-5ms CUDA accelerated neural inference", "🖥️"),
            ("Real-Time Prediction API", "Fraud alerts & defect flags via MLflow", "🎯")
        ],
        "backend_steps": [
...
<span style="color:#27c93f">[OK]</span> Zero Model Drift Detected in 24-Hour Cycle''',
    ...
    },
    {
        ...
        "vertical_nodes": [
            ("SaaS APIs & Kafka Streaming Ingress", "Extracts REST APIs, PostgreSQL, & sensor logs", "🌐"),
            ("Apache Airflow & dbt Core", "ELT transformation DAGs with data quality checks", "⚙️"),
            ("Snowflake Cloud Data Warehouse", "Columnar data lakehouse with ACID compliance", "🏢"),
            ("Real-Time BI Dashboard Feed", "Tableau & Looker analytics synchronization", "💡")
        ],
        "backend_steps": [
...
<span style="color:#27c93f">[OK]</span> Snowflake Data Warehouse Updated. BI Dashboard Refreshed.''',
    ...
    },
    {
        ...
        "vertical_nodes": [
            ("Next.js 19 React Client App", "SSR rendering, TailwindCSS, & GSAP animations", "💻"),
            ("FastAPI Backend Microservices", "Async Python endpoints with CORS & Pydantic", "🚪"),
            ("PostgreSQL & Redis Cache Layer", "ACID database & warm Redis query caching", "☁️"),
            ("Vercel Edge Global CDN", "Sub-10ms global edge HTML & JSON rendering", "🌍")
        ],
        "backend_steps": [
...
print("ULTRA-COMPACT ABOVE-THE-FOLD SERVICE OVERVIEW WITH REAL SYSTEM ARCHITECTURE DEPLOYED!")
