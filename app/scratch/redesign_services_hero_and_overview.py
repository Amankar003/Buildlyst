...
# 1. Update services_layout.html (Hero inspired by Home Page with Gentle Parallax Waves, Back Button, and Navigation)
...
<!-- REDESIGNED HERO SECTION (INSPIRED BY HOME PAGE HERO WITH ANIMATED WAVE CANVAS & BACK BUTTON) -->
<section class="hero-section reveal" style="min-height: auto; padding: 130px 0 60px 0; position: relative; overflow: hidden; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.06);">
    <!-- Animated Gradient Waves (Inspired by Home Page) -->
    <div class="wave-container">
        <svg class="waves" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
            viewBox="0 24 150 28" preserveAspectRatio="none" shape-rendering="auto">
            <defs>
                <path id="gentle-wave-service"
                    d="M-160 44c30 0 58-18 88-18s 58 18 88 18 58-18 88-18 58 18 88 18 v44h-352z" />
                <linearGradient id="wave-gradient-service" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <g class="parallax-waves">
                <use xlink:href="#gentle-wave-service" x="48" y="0" fill="url(#wave-gradient-service)" opacity="0.06" />
                <use xlink:href="#gentle-wave-service" x="48" y="3" fill="url(#wave-gradient-service)" opacity="0.12" />
                <use xlink:href="#gentle-wave-service" x="48" y="5" fill="url(#wave-gradient-service)" opacity="0.18" />
                <use xlink:href="#gentle-wave-service" x="48" y="7" fill="url(#wave-gradient-service)" opacity="0.28" />
            </g>
        </svg>
    </div>

    <div class="container" style="position: relative; z-index: 10; max-width: 960px;">
        
        <!-- BACK TO HOME PAGE BUTTON -->
        <div style="margin-bottom: 20px;">
            <a href="/" class="btn glass-btn" style="padding: 8px 18px; font-size: 13px; display: inline-flex; align-items: center; gap: 8px; border-radius: 50px; border: 1px solid rgba(0, 210, 255, 0.3); background: rgba(0, 210, 255, 0.05); color: #fff;">
                <span style="color: var(--c-accent-cyan); font-weight: bold; font-size: 16px;">&larr;</span> Back to Home Page
            </a>
        </div>

        <span class="overline highlight" style="font-size: 12px; letter-spacing: 2px;">BUILDLYST ENGINEERING SERVICES</span>
        
        <h1 class="text-gradient-hero" style="font-size: clamp(34px, 4.6vw, 56px); line-height: 1.12; margin: 14px 0 18px 0; font-weight: 800;">
            Silicon Valley-Grade Enterprise Architectures & Production Services
        </h1>
        
        <p class="subtext mx-auto" style="font-size: 16px; max-width: 740px; line-height: 1.65; color: var(--c-text-secondary); margin-bottom: 28px;">
            We don't build generic templates. Buildlyst architects custom AI agent swarms, private LLM RAG pipelines, sub-5ms machine learning models, and high-scale cloud data lakehouses — engineered with 100% source code ownership and SOC2 compliance.
        </p>
        ...
        <!-- 5 Service Navigation Pill Buttons -->
...
<div style="min-height: 100vh;">
    ...
    <section class="reveal" style="padding: 50px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
        <div class="container">
            <div style="display: grid; grid-template-columns: 1.15fr 0.85fr; gap: 36px; align-items: center;">
                
                <!-- LEFT SIDE: RICH SERVICE OVERVIEW CONTENT (CASE STUDY REMOVED, EXPANDED CONTENT ADDED) -->
                <div>
...
# 5 Detailed Services Data with EXPANDED SERVICE OVERVIEW CONTENT (Case Study Removed from Overview)
services_data = [
    {
...
app = workflow.compile()''',
        "terminal_logs": '''<span style="color:#00d2ff">[AGENT_KERNEL]</span> Ingesting Ingress User Payload...<br>
<span style="color:#27c93f">[MATCH]</span> Found 3 Matching Property Vectors (Pinecone Similarity: 0.96)<br>
<span style="color:#00d2ff">[CRM]</span> Updating Database Record #9042...<br>
<span style="color:#27c93f">[SUCCESS]</span> Dispatched Booking Link to Client Endpoint<br>
...
query_engine = index.as_query_engine(
    node_postprocessors=[reranker], streaming=True
        ...
        "terminal_logs": '''<span style="color:#00d2ff">[LEGAL_RAG]</span> Ingesting Master Document.pdf...<br>
...
if output['defect_score'] > 0.99:
    plc_eject_trigger.send_signal()''',
        "terminal_logs": '''<span style="color:#00d2ff">[ML_ENGINE]</span> RTSP Video Stream Ingested (60 FPS)<br>
...
with DAG('finscale_analytics_pipeline', schedule_interval='@hourly') as dag:
    ...
    ingest_kafka >> dbt_transform''',
        "terminal_logs": '''<span style="color:#00d2ff">[DATA_ENGINE]</span> Ingested 5,000 Kafka Events/sec<br>
...
async def get_analytics(user=Depends(verify_jwt_session)):
    ...
    return await db.fetch_tenant_analytics(user.tenant_id)''',
        "terminal_logs": '''<span style="color:#00d2ff">[WEB_ENGINE]</span> FastAPI Server Running on Port 8000<br>
<span style="color:#27c93f">[REDIS]</span> Cache Hit for Active Session (Latency: 1.2ms)<br>
...
print("SERVICES HERO REDESIGNED WITH HOME PARALLAX WAVES & OVERVIEW EXPANDED PERFECTLY!")
